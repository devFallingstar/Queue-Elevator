...
public class LPacman extends Thread{
	private JLabel lb;
	private LSection section;
	...
	boolean canBack;
	int preFloor;
	public LPacman(JLabel lb, JButton callBtn[],int currentSct,int spd){
		this.speed = spd;
		this.lb = lb;
		...
		SetIconToLabel.set(lb, "img/pacman_u.png", 75, 75);
	}
	...
	public int sectionToFloor(int sec){
		if(sec == 2) return 6;
		else if(sec == 3) return 5;
		else if(sec == 4) return 4;
		else if(sec == 5) return 3;
		else if(sec == 6) return 2;
		else if(sec == 7) return 1;
		if(sec == 8) return 0;
		return  -1;
	}
	...
	private void move(){
		...
		if(currentSct == 1)
			SetIconToLabel.set(lb, "img/pacman_r.png", 75, 75);
		else if(currentSct == 8) SetIconToLabel.set(lb, "img/pacman_l.png", 75, 75);
		else if(currentSct > 8 || currentSct < 1) SetIconToLabel.set(lb, "img/pacman_u.png", 75, 75);
		else SetIconToLabel.set(lb, "img/pacman_d.png", 75, 75);
		
		Point nextPoint;
		nextPoint = section.getSectionPoint((currentSct+1)%LSection.SQUARE_ROUTE_NUM);
